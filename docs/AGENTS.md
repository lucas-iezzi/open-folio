...
**Setup carousel progress (`~/.open-folio-setup-progress.json` on the remote):** The 13-step setup wizard's completed-step dots are tracked by a small JSON array of step indices written to `$HOME/.open-folio-setup-progress.json` on the server itself — deliberately in `$HOME`, not `remotePath`, since the latter is exactly the directory the git-clone step (see below) needs to find empty. `POST /admin/deploy/setup-progress` reads it (returns `{ configured: false }` if no server is saved yet), `POST /admin/deploy/mark-setup-step` (`{ step }`) reads-merges-writes it via `sshExecWithStdin`. `admin.js`'s `markStepDone()` calls the latter after every successful step (both SSH-run steps and the two manual "mark done" steps); `loadSetupProgress()` fetches current state on page load and again after saving new SSH credentials. This used to live in the browser's `localStorage`, which kept showing old progress as "done" after the actual server was wiped/rebuilt since nothing tied it to a specific server instance — the file-on-remote approach means a reset server has no file and correctly starts the checklist over. Once a step's own `doneSteps` entry is set, `updateRunButtonState()` keeps that step's run/mark-done button disabled — only Prev/Next can move off of it — so it can't be accidentally re-run; it re-enables automatically if that step's completion is ever missing (a failed run, or a fresh server with no progress file).

**Cloning the repo (`SSH_CMDS.git_clone`):** Re-running this step can't just `git clone` blindly — the destination might already be a working clone (pull it instead), or might have leftover content from an earlier attempt (git refuses to clone into a non-empty directory). If `remotePath/.git` exists, it's a `git pull`. Otherwise, if the directory exists and is non-empty, it's renamed aside to `<remotePath>.bak-<timestamp>` (never deleted), a fresh clone happens into the now-clean path, and any real content the old directory had — `.env`, and everything in `data/`, `public/images/projects/`, `public/images/logos/` except `.gitkeep` — is moved into the new clone. This is safe because git only ever tracks `.gitkeep` placeholders in those content directories (see `.gitignore`), so nothing a fresh clone brings in can collide with real content; the `.bak-*` directory is left behind afterward for the user to inspect and remove manually, not auto-deleted.
