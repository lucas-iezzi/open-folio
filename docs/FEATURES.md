...
- **Server Commands:** restart the site, view logs, check status (PM2 + disk + uptime), pull code updates (git pull + npm install + restart), change the live admin password or secret admin path, and **Clear Content** — wipes all projects + project images on just the server or just the local site (logos and site settings are kept), so that side can be pushed/pulled fresh. Backs up automatically first and requires typing "DELETE" to confirm.
- Opening the tab automatically expands whichever section needs attention: Server Credentials if nothing's saved yet or the connection fails, Server Setup if connected but the wizard isn't finished, or Content Sync (with the comparison already open and running) once setup is complete.
